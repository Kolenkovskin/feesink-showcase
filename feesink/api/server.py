...
<head>
    ...
    <title>FeeSink</title>
    <style>
        body {{ font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; margin: 40px; color: #111; background: #fff; }}
        .box {{ max-width: 760px; }}
        code {{ background:#f3f3f3; padding:2px 6px; border-radius:6px; }}
        .muted {{ color:#666; }}
        .row {{ margin-top: 14px; }}
        input {{ width: 100%; max-width: 560px; padding: 12px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; }}
        .btn {{ display: inline-block; margin-top: 10px; padding: 12px 18px; background: #111; color: #fff; border: 0; border-radius: 8px; font-weight: 700; cursor: pointer; }}
        .btn2 {{ display: inline-block; margin-top: 10px; margin-left: 8px; padding: 12px 18px; background: #f3f3f3; color: #111; border: 1px solid #ddd; border-radius: 8px; font-weight: 700; cursor: pointer; }}
        .err {{ margin-top: 10px; color: #b00020; white-space: pre-wrap; }}
        .ok {{ margin-top: 10px; color: #0b6b0b; white-space: pre-wrap; }}
...
<body>
    <div class="box">
        ...
        <div class="row">
            ...
            <div class="small muted">The token identifies your account. You create it yourself (self-issued).</div>
  ...
  -Body '{{
    ...
    "enabled": true
        ...
        <p class="muted" style="margin-top:18px;">
            ...
            Contract: <code>API_CONTRACT_v1.md</code><br/>
            {LANDING_VERSION}
        </p>
    </div>
    ...
    (function() {{
        ...
        document.getElementById("genBtn").addEventListener("click", function() {{ generateToken(); }});
        document.getElementById("copyBtn").addEventListener("click", function() {{ copyToken(); }});
        document.getElementById("payBtn").addEventListener("click", function() {{ createCheckout(); }});
    }})();
    </script>
</body>
</html>"""
